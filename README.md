...
> **温馨提示：** 如果你的 Agent 平台支持**记忆（Memory）功能**（如 CodeBuddy），建议在完成初始设定后让 Agent 将团本的核心信息写入记忆。这样在长团或多会话切换时，Agent 无需重新加载整个档案就能保持对角色、规则和当前进度的认知。写入内容建议包括：团本名称、玩家角色、副官身份、规则系统、当前场景位置。
