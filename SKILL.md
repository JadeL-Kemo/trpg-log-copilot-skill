...
| 步 | 产出 | 操作 | 🔴 提醒 |
|:--:|------|------|------|
| **0** | `00_当前局势.md` | read_file | 禁止跳过，扫 HP/SAN/场景 |
| **1** | `01_线索.md` | `file_ops append-table` | 每行：id content source verified confidence tags linked |
| **2** | `02_人物.md` | `file_ops append-table` | 新 NPC 才追加 |
| **2a** | `02_人物.md` | 检查 NPC 的 `relationships` 列，确认新互动→更新关系 | 🔴 关系变更必更新！格式：`对方(类型,direction)` |
| **3** | `03_时间线.md` | `file_ops append-table` | scene_id 必填！event_date 按精度 |
| **3a** | `03a_大纪事.md` | `file_ops append-table` | 仅 YYYY/YYYY-MM |
| **4** | `04_行动日志.md` | `file_ops append-scene` | `### SXX 标题` + 叙事 |
| **4a** | SQL | `state add <角色> --hp/-san +/-N --reason <名> --date MM-DD --time HH:MM --clue CL-XXX` | 🔴 HP/SAN 变更必调！--date --time 必填 |
| **5** | `06_待办.md` | `replace_in_file` | `- [ ] 优先级 任务 (原因) → CL-XXX` |
| **6** | 同步 | `import_md.py` | MD → SQL + 渲染 panel.html |
